...
export default function UsersPage() {
  ...
  const toggleActive = async (target: User) => {
    try {
      ...
      toast.success(
        target.isActive
          ? `${target.fullName} was deactivated.`
          : `${target.fullName} was reactivated.`,
      );
    } catch (error) {
  ...
  return (
    <div className="space-y-6 p-8">
      ...
      <SegmentedTabs
        tabs={[
          { value: "staff", label: "Staff accounts", icon: <UsersIcon className="size-4" /> },
          { value: "stewards", label: "Stewards", icon: <UserRound className="size-4" /> },
        ]}
        value={tab}
                ...
                <TableBody>
                  {users.map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className="text-sm text-muted-foreground">
                        {row.lastLoginAtUtc
                          ? new Date(row.lastLoginAtUtc).toLocaleString()
                          : "Never"}
                      </TableCell>
                      <TableCell>
                        <DropdownMenu>
